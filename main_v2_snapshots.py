    ...
    for i in tqdm(range(dataset_size)):
        for snr in SNR:
            ...
            for ss in range(n_snapshots):
                ...
                y_ = a(theta,r) * s + n # uplink received signal
                y = np.concatenate((y_.real, y_.imag))
                y_snap.append(y)
            y_snapshots = np.stack(y_snap)
...
if args.train:
    ...
    test_loss, RMSE_test = eval_loop(X_test, y_test, SNR_test, model, criterion, device,N,N_RF,range_limits, batch_size=30,type=args.type)
